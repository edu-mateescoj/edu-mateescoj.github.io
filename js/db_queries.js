...
*/
// CONSTANTE DE CONFIGURATION : Mettre à 'true' pour la version GitHub Pages (statique), 'false' pour la version Flask (dynamique avec BDD).
...
function logFactory(type, body) {
    // --- MODIFICATION POUR VERSION STATIQUE ---
    if (IS_STATIC_VERSION) {
        console.log(`[STATIC MODE] Skipping log for type: ${type}`);
        // On retourne une promesse résolue avec un succès simulé et un ID fictif
        return Promise.resolve({
            status: 'success',
            message: 'Static mode: logging skipped',
            code_id: 'static_code_id_' + Date.now() // ID unique fictif pour que le reste du code fonctionne
        });
    }
    // ------------------------------------------
